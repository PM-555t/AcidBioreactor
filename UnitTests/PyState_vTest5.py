        ...
        match state:
            ...
            case 71:
                if state != lastState:
                    ...
                    switchTimerFlag(myReactor,True,True) #major timer
                    switchTimerFlag(myReactor,False,True) #minor timer
                    runPump(addrExcessPump,pumpCal*100) #run pump for 5 L
                    justPumped = True
                    myReactor._pumpOrMsr = True #set state to pumping, not measuring
                    pumpWaitTimer(myReactor,False,(pumpCal*100)+60.0) #set minor timer to pump time + delay
            ...
            case 72:
                if state != lastState:
                    ...
                    switchTimerFlag(myReactor,True,True) #major timer
                    switchTimerFlag(myReactor,False,True) #minor timer
                    runPump(addrSWPump,pumpCal*100) #run pump for 5 L
                    justPumped = True
                    myReactor._pumpOrMsr = True #set state to pumping, not measuring
                    pumpWaitTimer(myReactor,False,(pumpCal*100)+60.0) #set minor timer to pump time + delay
